...
Multi-line code blocks are expanded into one row per physical code line before normal layout. Every expanded row retains the code block's source range. Each line is first tokenised by `internal/highlight`, whose `Lines(lang, text)` returns one token slice per physical line — keyword, string, comment, number, or plain — with block-comment and multiline-string state carried across line boundaries. `codeStyle` maps each token kind onto a code style (`StyleCodeKeyword`/`String`/`Comment`/`Number`); plain tokens keep the base `StyleCode`, so an unknown or absent language, which the lexer renders all-plain, produces the same single `StyleCode` run per line as before highlighting existed. The token runs — preceded by the four-space indent run — go through `packRuns`, not `hardSplit`, so a line wider than the row is split without dropping any character while every token keeps its style; the no-row-exceeds-width invariant is preserved because `packRuns` clips to the available width. `internal/highlight` depends only on the standard library and sits below `layout`, which imports it; nothing about the palette reaches it.
...
`decodeMouse` handles a sequence opening with `<`. It accepts only a press — final byte `M` — and masks the modifier bits out of the button field before reading it, since shift, meta and control say nothing about which event this is; a shift-click is still a click. What remains is either 0, the primary button, or `0x40` plus a direction — 0 and 1 for a vertical wheel notch, 2 and 3 for a horizontal one. Testing for those rather than against a list of rejects means a report this decoder has never seen is dropped rather than mistaken for a click. Everything it drops becomes `KeyNone`, which the viewer ignores, rather than `KeyEscape`: it was plainly a mouse report, and turning it into a keypress would be worse than dropping it.

Turning tracking on takes the wheel away from the terminal, which would otherwise scroll the alternate screen itself, so `KeyWheelUp` and `KeyWheelDown` have to reach the viewer or scrolling stops working altogether. `App.handle` dispatches them ahead of the mode switch and of the diff bindings, because the wheel is not a binding: it is the reader physically scrolling, and it means the same thing in the viewer, in a diff, and with a search prompt open. One notch moves `wheelRows` rendered rows through the same clamped `scroll` every other movement uses.

`KeyWheelLeft` and `KeyWheelRight` are the other half of the same capture, and they are the opposite kind of thing: nothing scrolls horizontally, so a sideways swipe is read as the gesture it is. `App.swipe` sends it where `<` and `>` go — `selectRelative` in diff mode, `goBack`/`goForward` in the viewer — and so it is dispatched in `handleNormalKey`, alongside the keys it stands in for, rather than ahead of the mode switch: a stray gesture should not pull the file out from under a half-typed search query.

A trackpad delivers one flick as a burst of notches, often dozens, and acting on each would run the whole changed-file list past in a single gesture. `swipe` therefore debounces: a notch acts only if it changes direction or arrives more than `swipeGap` (400 ms) after the previous one, and every notch pushes that deadline forward, so a flick and its momentum are one step. Counting notches instead would have penalised a tilt wheel, which sends them one at a time. The clock is `App.now`, injected the way `runEdit` and `runOpen` are, so a test plays a gesture out without sleeping; it is nil in production and `clock` falls back to `time.Now`.
...
- link following over fakes: the wheel scrolling in every mode and clamping at both ends, Tab and Shift-Tab cycling and wrapping, starting from the viewport and scrolling a selection into view, Escape clearing it, Enter opening a selection and still scrolling without one, resolution against the document's own directory, the four unfollowable targets each reporting and leaving the viewport put, external URLs reaching the injected opener and refused ones not, back and forward with position restored and the forward stack cleared, clicks on and off a link and outside the document, a sideways swipe going back and forward and through the changed-file list with one flick's burst counting once and being ignored while searching, the selection banded in the background without writing through the shared document and surviving search highlighting, and every link key inert in diff mode;
