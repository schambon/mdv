...
Fenced code blocks are syntax-highlighted when their fence names a supported language: `go`, `js`/`ts`, `python`, `c`/`cpp`, `java`, `rust`, `sh`, and `json`, plus common aliases. Highlighting is deliberately coarse — a single generic lexer colours keywords, strings, comments, and numbers, driven by a per-language table of comment markers, string delimiters, and reserved words. It is not a parser and makes no claim to be exhaustive. An unrecognised or absent language leaves every token plain, so the block renders exactly as it would without highlighting. Indented code blocks are never highlighted, having no info string to carry a language. In a Markdown diff, unchanged code highlights the same way, but a changed code block is banded whole rather than highlighted, since the diff renderer works from raw text (see §8).
...
Within a changed block, the words that differ are highlighted against the block's background, as they are in the line comparison, and Markdown styling is kept underneath: a changed word inside a link is still an underlined link pointing at the same target. Word highlighting is suppressed, and the whole block simply banded, when the two versions are too dissimilar to align word by word, when `--no-word-diff` is given, and in code blocks and tables, where the renderer works from the raw text and could not draw a mark that fell inside it.
