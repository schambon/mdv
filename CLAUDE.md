...
- Out of scope: stdin, file watching, inline HTML, local-link resolution, images, footnotes.
- **Syntax highlighting is bounded, like the Markdown.** `internal/highlight` is one generic C-family lexer plus a small per-language table (comment markers, string quotes, keyword set) for a fixed language set; it colours keywords/strings/comments/numbers and nothing finer. An unknown or absent fence language renders all-plain — byte-identical to no highlighting. Resist growing it into a real grammar or per-language parsers; that is the same non-goal as CommonMark.
...
- `internal/highlight` — the bounded code lexer. `Lines(lang, text)` returns one token slice per physical line, keeping `concat(tokens) == line`; state carries across newlines so block comments and multiline strings span rows. Below `layout`, stdlib-only.
...
- `internal/layout/meta.go` — the frontmatter key column, in package `layout` for the same reason as `table.go`: it sizes a run of blocks and reuses `run`, `wrapRuns`, `clipSpans` and `Width`.
...
- **Frontmatter is a fence, and an unterminated one is not frontmatter.** `frontMatter` fires only at `p.i == 0` and only when a closing `---`/`...` exists; otherwise it returns false with the scan position untouched, so a document opening with a real horizontal rule is unharmed. It parses no YAML — a line is cut at its first key colon and the value is literal text. The key lives in `Block.Prefix`, not in `Inlines`, which is what keeps the Markdown diff's word cut and `markInlines` working on a frontmatter value exactly as on any other block's text. One block per line, deliberately: it is what gives `v`, `Nearest` and the diff their per-key granularity, and a single block for the whole header would make every metadata edit look like a rewrite.
...
- **One outstanding read, tracked by the viewer goroutine** (`internal/app/pump.go`). A second queued read would still be pending when the user presses `v` and would steal the editor's first keystroke. The OSC 11 background probe (`terminal.QueryBackground`, driven by `app.resolveTheme`) reads on the same input, so it must run after `Enter` but *before* the pump starts — one bounded, self-contained read whose reply cannot then be mistaken for a keypress.
- **Theme switching is presentation-only.** `style.New` maps a `layout.Style` to SGR at *draw* time, so both `t` (`app.toggleTheme`) and the `auto` re-resolution after startup are a styler swap and a repaint — never a reflow. Nothing about the palette reaches `layout`, and the initial `render` runs before raw mode without knowing which theme will be chosen. Keep it that way: a theme that required relaying out would defeat the whole reason the switch is instant.
